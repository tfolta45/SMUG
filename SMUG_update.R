...
ui <- fluidPage(
  fluidRow(
    ...
    column(12, p("Welcome to SMUG. We know what model you need!", align = "center")),
    column(12, p("")),
    column(12, p("")),
    #column(12, p("Chose from various variable situations at your own volition.", align = "left")),
    column(12, p("Select the variables you wish to include from the drop-down menus.", align = "left")),
    column(12, p("The radio buttons determine the output displayed on screen.", align = "left")),
...
server <- function(input,output) {
  ...
  output$onequantonecat <- renderUI({ switch(input$tool4,
                                             ...
                                             'ANOVA / T-test' = verbatimTextOutput('anovattest'))
  })
  ...
  output$anovattest <- renderPrint({
    anova_data <- as.data.frame(mpg[,c(input$QuantVar2,input$CatVar)])
